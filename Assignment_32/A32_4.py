...
def FindDuplicate(DirectorytName):
...
def RemoveDubplicate(DirectortName):
    Ret=False
    ...
    fobj.write(Border+"\n")
    fobj.write("Total duplicate files found & removed : " + str(cnt) + "\n")
    fobj.write("This log file was created on : " + timeStamp + "\n")
    fobj.write("Time required for excustion : "+str((time.time()-start_time))+"\n")
    fobj.write(Border+"\n")
...
def main():
    if len(sys.argv)!=2:
        ...
        print("Valid Argumests Are : python A32_4.py DirctoryName")
        return
    
    DirectoryName=sys.argv[1]
    RemoveDubplicate(DirectoryName) 
